...
Пусть \({ \mathcal M }\) --- риманово многообразие с периодическим концом, отвечающим бесконечному циклическому накрытию \({ M }\) компактного многообразия \({ M_{\infty} }\), ассоциированному с классом когомологий \({ \gamma \in H(M_{\infty}; \mathbb Z) }\), двойственным (по Пуанкаре) ориентированному гладкому подмногообразию \({ Y }\). Тогда \({ \mathcal M }\) состоит из компактного основания \({ M_{0} }\) с границей \({ Y }\) и хвоста \({ M^{+} }\), представляющего из себя <<половину>> \({ M }\), отрезанную по \({ Y }\) (см. \cite{mrowka2015index}.) 
...
Бесконечная циклическая группа \({ \Pi \coloneqq \langle t \rangle }\) свободно действует на \({ M }\) естественным сдвигом \({ t W_{k} \coloneqq W_{k+1} }\). Для \({ k \in \mathbb Z_{+} }\) обозначим \({ M^{+}_{k} \coloneqq t^{k} M^{+} \subseteq M^{+} \subseteq \mathcal M }\).

Легко построить гладкую функцию \({ f : M \to \mathbb R }\) такую, что \begin{equation}
    \label{eq:quasi-periodicity_of_f}
    f(tx) = f(x) + 1\,.
\end{equation} Достаточно положить \({ f }\) гладкой ступенькой в односторонней трубчатой окрестности части границы \({ Y^{+} }\) в \({ W_{0} }\), продолжить \({ f }\) нулём в \({ W_{0} }\) и определить соотношением (\ref{eq:quasi-periodicity_of_f}) на всём \({ M }\). При этом \({ df }\) корректно определено на \({ M_{\infty} }\).
...
Рассмотрим случай, когда \({ g^{\infty} }\) имеет (невырожденные) неподвижные точки. Пусть \({ \left\{ x_{j} \right\} }\) --- неподвижные точки \({ g_{\infty} }\) (тоже невырожденные в силу Замечания~\ref{rm:fixed_points_are_projections}) и пусть \({ x_{j} }\) лежат вне \({ Y }\). Пусть \({ \omega }\) --- поток на \({ M_{\infty} }\) (в смысле \cite[Chapter III]{de2012differentiable}), для которого \[
    d \omega = \sum_{x_{j}} \mathcal L_{x_{j}} \delta_{x_{j}}\: dVol\,,
\] где \({ \mathcal L_{x_{j}} }\) есть классический вклад неподвижной точки \({ x_{j} }\), а \({ \delta_{x_{j}} }\) --- \({ \delta }\)-функция, сосредоточенная в точке \({ x_{j} }\). Тогда, по аналогии с \cite[Theorem A]{mrowka2016index}, формула для числа Лефшеца, вероятно, принимает вид \[
    \mathcal L = \mathcal L_{\operatorname{int}} + \mathcal L' + \mathcal L_{\operatorname{sing}}\,,
\] где \[
...
\begin{remark}
    \({ \mathcal L' }\) не зависит от выбора \({ \omega }\), поскольку для \({ d\omega = d(\omega + \omega') }\) в силу формулы интегрирования по частям и определения функции \({ f }\) имеем \[
        \int_{M_{\infty}} df \land \omega' - \int_{Y} \omega' = -\int_{M_{\infty}} f \land \underbrace{d\omega'}_{0} + \int_{Y} (\underbrace{ft - f}_{1}) \omega' - \int_{Y} \omega' = 0\,.
    \]
\end{remark}

\begin{remark}
    Из формулы Стокса (если она справедлива для потоков) следует, что если при построении \({ \mathcal M }\) вместо \({ Y }\) взять какой-то другой разрез \({ \widetilde Y }\), то к \({ \mathcal L' }\) прибавляется сумма вкладов неподвижных точек, заключённых между \({ Y }\) и \({ \widetilde Y }\).
\end{remark}
