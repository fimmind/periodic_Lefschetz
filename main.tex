...
Пусть \({ \mathcal M }\) --- риманово многообразие с периодическим концом, отвечающим бесконечному циклическому накрытию \({ M }\) компактного многообразия \({ M_{\infty} }\), ассоциированному с классом когомологий \({ \gamma \in (M_{\infty}; \mathbb Z) }\), двойственным (по Пуанкаре) ориентированному гладкому подмногообразию \({ Y }\). Тогда \({ \mathcal M }\) состоит из компактного основания \({ M_{0} }\) с границей \({ Y }\) и хвоста \({ M^{+} }\), представляющего из себя <<половину>> \({ M }\), отрезанную по поднятию \({ Y }\) (см. \cite{mrowka2015index}.) 

При этом \({ M }\) можно представить как склейку по отображениям \({ Y^{-} \to Y^{+} }\) счётного числа фундаментальных областей \({ W_{k} }\), изометрически изоморфных \({ M_{\infty} \setminus Y }\) и с двумя берегами разреза \({ Y^{\pm} }\).
Бесконечная циклическая группа \({ \Pi \coloneqq \langle t \rangle }\) свободно действует на \({ M }\). Для \({ k \in \mathbb Z_{+} }\) обозначим \[
    M^{+}_{k} \coloneqq t^{k} M^{+} \subseteq M^{+} \subseteq \mathcal M\,.
\]
...
\begin{theorem}
    Пусть \({ g }\) имеет только невырожденные неподвижные точки, а \({ g_{\infty} }\) не имеет неподвижных точек. Тогда число Лефшеца \({ 
...
Рассмотрим случай, когда \({ g^{\infty} }\) имеет (невырожденные) неподвижные точки. Пусть \({ \left\{ x_{j} \right\} }\) --- неподвижные точки \({ g_{\infty} }\) (тоже невырожденные в силу замечания) и пусть \({ x_{j} }\) лежат вне разреза \({ Y }\). Пусть \({ \omega }\) --- поток (в смысле \cite{de2012differentiable}), для которого \[
    d \omega = \sum_{x_{j}} \mathcal L_{x_{j}} \delta_{x_{j}}\: dVol\,,
\] где \({ \mathcal L_{x_{j}} }\) есть вклад точки \({ x_{j} }\), а \({ \delta_{x_{j}} }\) --- \({ \delta }\)-функция, сосредоточенная в точке \({ x_{j} }\). Тогда, по аналогии с \cite[Theorem A]{mrowka2016index}, формула для числа Лефшеца, вероятно, принимает вид \[
...
\begin{remark}
    Из формулы Стокса следует, что если при построении \({ \mathcal M }\) вместо \({ Y }\) взять какой-то другой разрез \({ \widetilde Y }\), то к \({ \mathcal L' }\) прибавляется сумма вкладов неподвижных точек, заключённых между \({ Y }\) и \({ \widetilde Y }\).
\end{remark}
