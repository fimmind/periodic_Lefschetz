...
\] Для краткости будем опускать зависимость от \({ \omega }\) и писать, например, \({ B(-i \partial_{t}) }\) вместо \({ B(-i \partial_{t}, \omega, -i \partial_{\omega}) }\). Тогда вклад неподвижной точки \({ \alpha }\) в формуле Лефшеца выражается формулой \[
