...
Обозначим за \({ B = B(-i \partial_{t}, \omega, -i \partial_{\omega}) }\) псевдодифференциальный оператор с конормальным символом \[
    \sigma_{c}(D)^{-1}(p) \frac{\partial \sigma_{c}(D)}{\partial p}(p)\,.
    \] Для краткости будем опускать зависимость от \({ \omega }\) и писать, например, \({ B(-i \partial_{t}) }\) вместо \({ B(-i \partial_{t}, \omega, -i \partial_{\omega}) }\). Тогда, согласно \cite[Theorem 11.3]{nazaikinskii2005elliptic} (см. так же \cite{nazaikinskii1999atiyah}), вклад неподвижной точки \({ \alpha }\) в формуле Лефшеца выражается формулой \[
...
\] где след \({ \tau }\) отвечает вложению \[
    \Omega \hookrightarrow \mathbb R_{t} \times \Omega\,, \quad \omega \mapsto (0, \omega)\,.
\]
